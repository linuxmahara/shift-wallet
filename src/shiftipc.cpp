 ...
 * @date 2015
 *
 * Shift/Ethereum IPC client implementation
 */
...
// *************************** EtherIPC **************************** //
    ...
    void EtherIPC::connectionTimeout() {
        if ( !fAborted && fSocket.state() != QLocalSocket::ConnectedState ) {
            fSocket.abort();
            setError("Unable to establish IPC connection to Shift. Make sure Shift is running and try again.");
            bail();
        }
    }
    ...
    void EtherIPC::handleUnlockAccount() {
        ...
        if ( !result ) {
            ...
            if ( parseVersionNum() == 100002 ) {
                fError += " Shift v1.0.2 has a bug with unlocking empty password accounts! Consider updating";
            }
            emit error();
        }
    ...
    int EtherIPC::parseVersionNum() const {
        QRegExp reg("^Shift/v([0-9]+)\\.([0-9]+)\\.([0-9]+).*$");
        reg.indexIn(fClientVersion);
        if ( reg.captureCount() == 3 ) try { // it's Shift
